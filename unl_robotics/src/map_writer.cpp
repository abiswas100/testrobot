...
namespace {
  ...
  std::vector<UNL_Robotics::Point2D> generateLinePixels(double x1, double y1, double x2, double y2)
  {
    std::vector<UNL_Robotics::Point2D> pixels;
    ...
    for(int x=static_cast<int>(x1); x<=maxX; ++x) {
      ...
      if(steep)
        pixels.push_back(UNL_Robotics::Point2D{static_cast<double>(y), static_cast<double>(x)});
      else
        pixels.push_back(UNL_Robotics::Point2D{static_cast<double>(x), static_cast<double>(y)});
...
UNL_Robotics::map_writer::map_writer()
{
}
...
bool UNL_Robotics::map_writer::insertObject(const std::string& yaml_filepath,
...
void UNL_Robotics::map_writer::plotLine(const UNL_Robotics::Point2D& pt1, const UNL_Robotics::Point2D& pt2)
{
